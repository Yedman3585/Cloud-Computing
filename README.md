...
| Shared firewall policy and routing integration | Created the Ansible routing role and the accompanying configuration, then treated `ansible/group_vars/all.yml` and `ansible/roles/routing/**` as shared final-integration work during polishing: firewall policy/routing behavior, route compatibility, and interaction with the complete Docker/HA lab. | Treated `ansible/group_vars/all.yml` and `ansible/roles/routing/**` as shared final-integration work: maintained the Ansible data model, connected policy data to nftables rendering, connected routes through VIPs in the deployment workflow, and verified the behavior with deployment and tests. |
